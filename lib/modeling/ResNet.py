...
def add_ResNet_roi_conv5_light_head_shared_weights(model, blob_in, dim_in, spatial_scale):
    """Adds an RoI feature transformation (e.g., RoI pooling) followed by a
    res5/conv5 head applied to each RoI."""
    light_head = []
    k_min = 0
    for lvl in range(len(blob_in)):
        slvl = str(lvl)
        if lvl == k_min:
            x1_1 = model.Conv(
                blob_in=blob_in[lvl],
                blob_out='convx1_1_blob' + slvl,
                dim_in=cfg.FPN.DIM,
                dim_out=100,
                kernel=7,
                pad=3,
	    ...
	    x1_1 = model.Relu(x1_1, x1_1)
            ...
            x1_1 = model.ConvShared(
                ...
                pad=3,
                stride=1,
                weight='convx1_1_blob' + sk_min + '_w',
                bias='convx1_1_blob' + sk_min + '_b'
            )
            x1_1 = model.Relu(x1_1, x1_1)
        blob_in[lvl] = copy.deepcopy(x1_1)
    ...
    fc7 = model.FC(roi_feat, 'fc7', 100 * roi_size * roi_size, 2048)
